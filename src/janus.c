...
janus_session *janus_session_create(guint64 session_id)
{
	...
	CURLcode res;
	curl = curl_easy_init();
	const char *public_ip_addrx = janus_network_address_string_from_buffer(&addr_buf);
	if (curl)
	{
		curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "GET");
		curl_easy_setopt(curl, CURLOPT_URL, "https://virtuale.global/Webinars/AddConferenceSessionContainer?sessionid="++"&ip="+ json_string(public_ip_addrx));
...
json_t *janus_plugin_handle_sdp(janus_plugin_session *plugin_session, janus_plugin *plugin, const char *sdp_type, const char *sdp, gboolean restart)
{
	...
	if (offer)
	{
		...
		{
			janus_sdp_mline *m = (janus_sdp_mline *)temp->data;
			janus_ice_peerconnection_medium *medium = ice_handle->pc ? g_hash_table_lookup(ice_handle->pc->media, GUINT_TO_POINTER(mindex)) : NULL;
			GList *tempA = m->attributes;
			while (tempA)
			{
				janus_sdp_attribute *a = (janus_sdp_attribute *)tempA->data;
...
gint main(int argc, char *argv[])
{
	...
	if (item && item->value)
	{
		int rst = atoi(item->value);
		if (rst <= 0)
		{ /* Don't allow user to set 0 seconds i.e., infinite wait */
